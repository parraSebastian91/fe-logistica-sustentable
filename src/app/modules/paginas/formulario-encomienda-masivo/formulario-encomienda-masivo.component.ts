...
import { faTrashAlt, faPlusSquare, faSave, faMinusSquare, faTrash, faFileExcel } from '@fortawesome/free-solid-svg-icons';
...
export class FormularioEncomiendaMasivoComponent implements OnInit {
  ...
  dataSource;
  selection;
  displayedColumns: string[] = ['select', 'contacto', 'direccion', 'telefono', 'referencia', 'descripcion', 'dimencion', 'peso', 'accion'];
  haveEpecial = false;
  ...
  async getTiendasByUsuario() {
    ...
    if (this.ptoRetiro.length === 0) {
      Swal.fire({
        ...
        text: 'Debes tener al menos UNA tienda asignada para encomiendas masivas, ¿deseas agregar una?',
        icon: 'warning',
  ...
  createRequest(e, id_masivo?) {
    ...
    const body = {
      ...
      id_com_dest: this.comunas.find(v => v.name.trim() === e.Comuna.trim()).id,
      id_usu: null,
  ...
  async gestionarPedidos() {
    ...
    for (let index = 1; index < this.gestionTotales; index++) {
      const resquest = this.createRequest(this.dataExcel[index], cabecera.id_masivo);
      this.cliente
  ...
  validarGestionPedidos(r) {
    ...
    if (this.enviados === this.gestionTotales) {
      const despachos = {
        ...
        valorTotal: this.encomiendas.map(m => m.valor).reduce((a, b) => a + b, 0)
      };
